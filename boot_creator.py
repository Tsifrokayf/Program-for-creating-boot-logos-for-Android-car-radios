...
class BootAnimApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        # Подпись автора (добавлен пробел на конце, чтобы курсивная 'f' не обрезалась)
    ...
    def update_preview_from_settings(self, *args):
        # Эта функция вызывается при любом изменении параметров, 
        # влияющих на отображение (новый файл, изменение разрешения, смена режима).
        ...
        try:
            ...
            return  # Пока вбивают не числа, ничего не делаем
    ...
    def load_preview(self, path, target_w, target_h, mode):
        try:
            self.is_animating = False
            # Отменяем текущий цикл анимации, если он есть
            ...
            # Извлекаем максимум 30 кадров для легкого предпросмотра
            max_preview = min(frames_count, 30)
            
            # Определяем размер самого окна предпросмотра (200x200 max)
            # Чтобы показать реальную картинку, нам нужно отмасштабировать 
            # РЕЗУЛЬТИРУЮЩИЙ прямоугольник target_w x target_h так, чтобы он влез в 200x200.
            ...
            for i, frame in enumerate(ImageSequence.Iterator(gif)):
                ...
                # 1. Сначала генерируем кадр так, как он будет выглядеть на магнитоле
                frame = frame.convert("RGBA")
                
                # Черный фон экрана магнитолы (в натуральную величину)
                ...
                if mode == "stretch":
                    ...
                    # Нужно скопировать frame, иначе thumbnail меняет оригинал на 2-м кадре
                ...
                # 2. Теперь масштабируем результат (экран магнитолы) под размер 200x200 для интерфейса
                display_img = simulated_screen.resize((display_w, display_h), Image.Resampling.LANCZOS)
                
                # 3. Накладываем на прозрачный/серый фон превью-контейнера 200x200 для центровки
                bg = Image.new("RGBA", (200, 200), (40, 40, 40, 255)) # цвет превью-блока
                bx = (200 - display_w) // 2
                by = (200 - display_h) // 2
                bg.paste(display_img, (bx, by))
                
                # CTkImage для CustomTkinter
    ...
    def start_generation(self):
        ...
        self.is_animating = False # Останавливаем превью для экономии ресурсов
        if hasattr(self, 'anim_job'):
            self.after_cancel(self.anim_job)
    ...
    def process(self, gif_path, out_zip, target_w, target_h, fps, mode, loop, color_depth):
        ...
        try:
            ...
            for frame in ImageSequence.Iterator(gif):
                frame = frame.convert("RGBA")
                # Для фона используем черный цвет с нужной глубиной
                bg = Image.new(color_depth, (target_w, target_h), (0, 0, 0))
                
                if mode == "stretch":
                    resized = frame.resize((target_w, target_h), Image.Resampling.LANCZOS)
                    # Если RGB, конвертируем перед вставкой (откидываем альфу корректно)
    ...
    def finish(self, success, msg):
        ...
        # Перезапускаем превью, если файл всё еще выбран
